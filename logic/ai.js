...
function buildPrompt(divination, question) {
    ...
    const linesDesc = original.lines.map((line, idx) => {
...
async function describeHttpError(response, endpoint) {
    let detail = '';
    try {
...
async function streamCompletion(config, userPrompt, callbacks, signal) {
    ...
    try {
        ...
        callbacks.onError(`无法连接到接口地址${hint}。常见原因：该端点未开放浏览器跨域（CORS）访问、地址填写有误，或网络被拦截。｜请求地址：${endpoint}`);
        return;
    }
    if (!response.ok) {
    ...
    const drain = (final) => {
        ...
        for (const line of lines) {
            ...
            const inlineError = asText(readPath(payload, 'error', 'message')) ?? asText(readPath(payload, 'error'));
            if (inlineError) {
    ...
    if (inlineFailure)
        callbacks.onError(inlineFailure);
    else
        callbacks.onDone();
}
async function aiDivination(divination, question, config, callbacks, signal) {
    if (!(0, exports.isAiConfigured)(config)) {
...
async function testAiConnection(config) {
    ...
    let received = false;
    let failure = '';
    await streamCompletion(config, '这是一次连通性测试，请只回复两个字：可用。', {
        onToken: () => {
            if (received)
                return;
            received = true;
    ...
    if (received)
        return { ok: true, message: `连接成功，${config.model.trim()} 可正常调用` };
