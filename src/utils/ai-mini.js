...
const cleanModelContent = (value) => value
...
function describeHttpError(responseData, status, endpoint) {
    ...
    if (status === 401 || status === 403)
        ...
        reason = '请求服务端异常';
    return `${reason}（HTTP ${status}）${detail ? `：${detail}` : ''}`;
}
...
function requestCompletion(config, userPrompt, callbacks) {
    ...
    if (/^minimax-/i.test(payload.model)) {
    ...
    const req = wx.request({
        ...
        fail: (err) => {
            ...
            callbacks.onError(`无法连接到接口地址${hint}。常见原因：该端点不可访问，请核对地址和网络。`);
        },
    });
    return {
        abort: () => {
