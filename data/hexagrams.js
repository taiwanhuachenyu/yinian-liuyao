"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.HEXAGRAMS = void 0;
exports.getHexagramById = getHexagramById;
const trigrams_1 = require("./trigrams");
const QIAN = trigrams_1.TRIGRAMS[0];
const DUI = trigrams_1.TRIGRAMS[1];
const LI = trigrams_1.TRIGRAMS[2];
const ZHEN = trigrams_1.TRIGRAMS[3];
const XUN = trigrams_1.TRIGRAMS[4];
const KAN = trigrams_1.TRIGRAMS[5];
const GEN = trigrams_1.TRIGRAMS[6];
const KUN = trigrams_1.TRIGRAMS[7];
exports.HEXAGRAMS = [
    {
        id: 1, name: '乾为天', symbol: '䷀', upperTrigram: QIAN, lowerTrigram: QIAN,
        judgment: '乾：元，亨，利，贞。',
        lines: [
            { position: 1, text: '初九：潜龙勿用。' },
            { position: 2, text: '九二：见龙在田，利见大人。' },
            { position: 3, text: '九三：君子终日乾乾，夕惕若厉，无咎。' },
            { position: 4, text: '九四：或跃在渊，无咎。' },
            { position: 5, text: '九五：飞龙在天，利见大人。' },
            { position: 6, text: '上九：亢龙有悔。' },
        ],
    },
    {
        id: 2, name: '坤为地', symbol: '䷁', upperTrigram: KUN, lowerTrigram: KUN,
        judgment: '坤：元，亨，利牝马之贞。君子有攸往，先迷后得主，利西南得朋，东北丧朋。安贞，吉。',
        lines: [
            { position: 1, text: '初六：履霜，坚冰至。' },
            { position: 2, text: '六二：直，方，大，不习无不利。' },
            { position: 3, text: '六三：含章可贞。或从王事，无成有终。' },
            { position: 4, text: '六四：括囊，无咎无誉。' },
            { position: 5, text: '六五：黄裳，元吉。' },
            { position: 6, text: '上六：龙战于野，其血玄黄。' },
        ],
    },
    {
        id: 3, name: '水雷屯', symbol: '䷂', upperTrigram: KAN, lowerTrigram: ZHEN,
        judgment: '屯：元，亨，利，贞，勿用有攸往，利建侯。',
        lines: [
            { position: 1, text: '初九：磐桓；利居贞，利建侯。' },
            { position: 2, text: '六二：屯如邅如，乘马班如。匪寇婚媾，女子贞不字，十年乃字。' },
            { position: 3, text: '六三：即鹿无虞，惟入于林中，君子几不如舍，往吝。' },
            { position: 4, text: '六四：乘马班如，求婚媾，往吉无不利。' },
            { position: 5, text: '九五：屯其膏，小贞吉，大贞凶。' },
            { position: 6, text: '上六：乘马班如，泣血涟如。' },
        ],
    },
    {
        id: 4, name: '山水蒙', symbol: '䷃', upperTrigram: GEN, lowerTrigram: KAN,
        judgment: '蒙：亨。匪我求童蒙，童蒙求我。初筮告，再三渎，渎则不告。利贞。',
        lines: [
            { position: 1, text: '初六：发蒙，利用刑人，用说桎梏，以往吝。' },
            { position: 2, text: '九二：包蒙吉，纳妇吉，子克家。' },
            { position: 3, text: '六三：勿用娶女；见金夫，不有躬，无攸利。' },
            { position: 4, text: '六四：困蒙，吝。' },
            { position: 5, text: '六五：童蒙，吉。' },
            { position: 6, text: '上九：击蒙；不利为寇，利御寇。' },
        ],
    },
    {
        id: 5, name: '水天需', symbol: '䷄', upperTrigram: KAN, lowerTrigram: QIAN,
        judgment: '需：有孚，光亨，贞吉。利涉大川。',
        lines: [
            { position: 1, text: '初九：需于郊。利用恒，无咎。' },
            { position: 2, text: '九二：需于沙。小有言，终吉。' },
            { position: 3, text: '九三：需于泥，致寇至。' },
            { position: 4, text: '六四：需于血，出自穴。' },
            { position: 5, text: '九五：需于酒食，贞吉。' },
            { position: 6, text: '上六：入于穴，有不速之客三人来，敬之终吉。' },
        ],
    },
    {
        id: 6, name: '天水讼', symbol: '䷅', upperTrigram: QIAN, lowerTrigram: KAN,
        judgment: '讼：有孚，窒。惕中吉。终凶。利见大人，不利涉大川。',
        lines: [
            { position: 1, text: '初六：不永所事，小有言，终吉。' },
            { position: 2, text: '九二：不克讼，归而逋，其邑人三百户，无眚。' },
            { position: 3, text: '六三：食旧德，贞厉，终吉；或从王事，无成。' },
            { position: 4, text: '九四：不克讼，复即命，渝，安贞吉。' },
            { position: 5, text: '九五：讼，元吉。' },
            { position: 6, text: '上九：或锡之鞶带，终朝三褫之。' },
        ],
    },
    {
        id: 7, name: '地水师', symbol: '䷆', upperTrigram: KUN, lowerTrigram: KAN,
        judgment: '师：贞，丈人，吉无咎。',
        lines: [
            { position: 1, text: '初六：师出以律，否臧凶。' },
            { position: 2, text: '九二：在师中吉，无咎，王三锡命。' },
            { position: 3, text: '六三：师或舆尸，凶。' },
            { position: 4, text: '六四：师左次，无咎。' },
            { position: 5, text: '六五：田有禽，利执言，无咎。长子帅师，弟子舆尸，贞凶。' },
            { position: 6, text: '上六：大君有命，开国承家，小人勿用。' },
        ],
    },
    {
        id: 8, name: '水地比', symbol: '䷇', upperTrigram: KAN, lowerTrigram: KUN,
        judgment: '比：吉。原筮元永贞，无咎。不宁方来，后夫凶。',
        lines: [
            { position: 1, text: '初六：有孚比之，无咎。有孚盈缶，终来有他，吉。' },
            { position: 2, text: '六二：比之自内，贞吉。' },
            { position: 3, text: '六三：比之匪人。' },
            { position: 4, text: '六四：外比之，贞吉。' },
            { position: 5, text: '九五：显比，王用三驱，失前禽，邑人不诫，吉。' },
            { position: 6, text: '上六：比之无首，凶。' },
        ],
    },
    {
        id: 9, name: '风天小畜', symbol: '䷈', upperTrigram: XUN, lowerTrigram: QIAN,
        judgment: '小畜：亨。密云不雨，自我西郊。',
        lines: [
            { position: 1, text: '初九：复自道，何其咎，吉。' },
            { position: 2, text: '九二：牵复，吉。' },
            { position: 3, text: '九三：舆说辐，夫妻反目。' },
            { position: 4, text: '六四：有孚，血去惕出，无咎。' },
            { position: 5, text: '九五：有孚挛如，富以其邻。' },
            { position: 6, text: '上九：既雨既处，尚德载，妇贞厉。月几望，君子征凶。' },
        ],
    },
    {
        id: 10, name: '天泽履', symbol: '䷉', upperTrigram: QIAN, lowerTrigram: DUI,
        judgment: '履：履虎尾，不咥人，亨。',
        lines: [
            { position: 1, text: '初九：素履，往无咎。' },
            { position: 2, text: '九二：履道坦坦，幽人贞吉。' },
            { position: 3, text: '六三：眇能视，跛能履，履虎尾，咥人，凶。武人为于大君。' },
            { position: 4, text: '九四：履虎尾，愬愬终吉。' },
            { position: 5, text: '九五：夬履，贞厉。' },
            { position: 6, text: '上九：视履考祥，其旋元吉。' },
        ],
    },
    {
        id: 11, name: '地天泰', symbol: '䷊', upperTrigram: KUN, lowerTrigram: QIAN,
        judgment: '泰：小往大来，吉亨。',
        lines: [
            { position: 1, text: '初九：拔茅茹，以其汇，征吉。' },
            { position: 2, text: '九二：包荒，用冯河，不遐遗，朋亡，得尚于中行。' },
            { position: 3, text: '九三：无平不陂，无往不复，艰贞无咎。勿恤其孚，于食有福。' },
            { position: 4, text: '六四：翩翩不富，以其邻，不戒以孚。' },
            { position: 5, text: '六五：帝乙归妹，以祉元吉。' },
            { position: 6, text: '上六：城复于隍，勿用师。自邑告命，贞吝。' },
        ],
    },
    {
        id: 12, name: '天地否', symbol: '䷋', upperTrigram: QIAN, lowerTrigram: KUN,
        judgment: '否：否之匪人，不利君子贞，大往小来。',
        lines: [
            { position: 1, text: '初六：拔茅茹，以其汇，贞吉亨。' },
            { position: 2, text: '六二：包承。小人吉，大人否亨。' },
            { position: 3, text: '六三：包羞。' },
            { position: 4, text: '九四：有命无咎，畴离祉。' },
            { position: 5, text: '九五：休否，大人吉。其亡其亡，系于苞桑。' },
            { position: 6, text: '上九：倾否，先否后喜。' },
        ],
    },
    {
        id: 13, name: '天火同人', symbol: '䷌', upperTrigram: QIAN, lowerTrigram: LI,
        judgment: '同人：同人于野，亨。利涉大川，利君子贞。',
        lines: [
            { position: 1, text: '初九：同人于门，无咎。' },
            { position: 2, text: '六二：同人于宗，吝。' },
            { position: 3, text: '九三：伏戎于莽，升其高陵，三岁不兴。' },
            { position: 4, text: '九四：乘其墉，弗克攻，吉。' },
            { position: 5, text: '九五：同人，先号啕而后笑。大师克相遇。' },
            { position: 6, text: '上九：同人于郊，无悔。' },
        ],
    },
    {
        id: 14, name: '火天大有', symbol: '䷍', upperTrigram: LI, lowerTrigram: QIAN,
        judgment: '大有：元亨。',
        lines: [
            { position: 1, text: '初九：无交害，匪咎，艰则无咎。' },
            { position: 2, text: '九二：大车以载，有攸往，无咎。' },
            { position: 3, text: '九三：公用亨于天子，小人弗克。' },
            { position: 4, text: '九四：匪其彭，无咎。' },
            { position: 5, text: '六五：厥孚交如，威如；吉。' },
            { position: 6, text: '上九：自天祐之，吉无不利。' },
        ],
    },
    {
        id: 15, name: '地山谦', symbol: '䷎', upperTrigram: KUN, lowerTrigram: GEN,
        judgment: '谦：亨，君子有终。',
        lines: [
            { position: 1, text: '初六：谦谦君子，用涉大川，吉。' },
            { position: 2, text: '六二：鸣谦，贞吉。' },
            { position: 3, text: '九三：劳谦君子，有终吉。' },
            { position: 4, text: '六四：无不利，撝谦。' },
            { position: 5, text: '六五：不富以其邻，利用侵伐，无不利。' },
            { position: 6, text: '上六：鸣谦，利用行师，征邑国。' },
        ],
    },
    {
        id: 16, name: '雷地豫', symbol: '䷏', upperTrigram: ZHEN, lowerTrigram: KUN,
        judgment: '豫：利建侯行师。',
        lines: [
            { position: 1, text: '初六：鸣豫，凶。' },
            { position: 2, text: '六二：介于石，不终日，贞吉。' },
            { position: 3, text: '六三：盱豫，悔。迟有悔。' },
            { position: 4, text: '九四：由豫，大有得，勿疑，朋盍簪。' },
            { position: 5, text: '六五：贞疾，恒不死。' },
            { position: 6, text: '上六：冥豫，成有渝，无咎。' },
        ],
    },
    {
        id: 17, name: '泽雷随', symbol: '䷐', upperTrigram: DUI, lowerTrigram: ZHEN,
        judgment: '随：元，亨，利，贞，无咎。',
        lines: [
            { position: 1, text: '初九：官有渝，贞吉。出门交有功。' },
            { position: 2, text: '六二：系小子，失丈夫。' },
            { position: 3, text: '六三：系丈夫，失小子。随有求得，利居贞。' },
            { position: 4, text: '九四：随有获，贞凶。有孚在道，以明，何咎。' },
            { position: 5, text: '九五：孚于嘉，吉。' },
            { position: 6, text: '上六：拘系之，乃从维之。王用亨于西山。' },
        ],
    },
    {
        id: 18, name: '山风蛊', symbol: '䷑', upperTrigram: GEN, lowerTrigram: XUN,
        judgment: '蛊：元，亨，利涉大川。先甲三日，后甲三日。',
        lines: [
            { position: 1, text: '初六：干父之蛊，有子，考无咎，厉终吉。' },
            { position: 2, text: '九二：干母之蛊，不可贞。' },
            { position: 3, text: '九三：干父之蛊，小有悔，无大咎。' },
            { position: 4, text: '六四：裕父之蛊，往见吝。' },
            { position: 5, text: '六五：干父之蛊，用誉。' },
            { position: 6, text: '上九：不事王侯，高尚其事。' },
        ],
    },
    {
        id: 19, name: '地泽临', symbol: '䷒', upperTrigram: KUN, lowerTrigram: DUI,
        judgment: '临：元，亨，利，贞。至于八月有凶。',
        lines: [
            { position: 1, text: '初九：咸临，贞吉。' },
            { position: 2, text: '九二：咸临，吉无不利。' },
            { position: 3, text: '六三：甘临，无攸利。既忧之，无咎。' },
            { position: 4, text: '六四：至临，无咎。' },
            { position: 5, text: '六五：知临，大君之宜，吉。' },
            { position: 6, text: '上六：敦临，吉无咎。' },
        ],
    },
    {
        id: 20, name: '风地观', symbol: '䷓', upperTrigram: XUN, lowerTrigram: KUN,
        judgment: '观：盥而不荐，有孚颙若。',
        lines: [
            { position: 1, text: '初六：童观，小人无咎，君子吝。' },
            { position: 2, text: '六二：窥观，利女贞。' },
            { position: 3, text: '六三：观我生，进退。' },
            { position: 4, text: '六四：观国之光，利用宾于王。' },
            { position: 5, text: '九五：观我生，君子无咎。' },
            { position: 6, text: '上九：观其生，君子无咎。' },
        ],
    },
    {
        id: 21, name: '火雷噬嗑', symbol: '䷔', upperTrigram: LI, lowerTrigram: ZHEN,
        judgment: '噬嗑：亨。利用狱。',
        lines: [
            { position: 1, text: '初九：屦校灭趾，无咎。' },
            { position: 2, text: '六二：噬肤灭鼻，无咎。' },
            { position: 3, text: '六三：噬腊肉，遇毒；小吝，无咎。' },
            { position: 4, text: '九四：噬干胏，得金矢，利艰贞，吉。' },
            { position: 5, text: '六五：噬干肉，得黄金，贞厉，无咎。' },
            { position: 6, text: '上九：何校灭耳，凶。' },
        ],
    },
    {
        id: 22, name: '山火贲', symbol: '䷕', upperTrigram: GEN, lowerTrigram: LI,
        judgment: '贲：亨。小利有攸往。',
        lines: [
            { position: 1, text: '初九：贲其趾，舍车而徒。' },
            { position: 2, text: '六二：贲其须。' },
            { position: 3, text: '九三：贲如濡如，永贞吉。' },
            { position: 4, text: '六四：贲如皤如，白马翰如，匪寇婚媾。' },
            { position: 5, text: '六五：贲于丘园，束帛戋戋，吝，终吉。' },
            { position: 6, text: '上九：白贲，无咎。' },
        ],
    },
    {
        id: 23, name: '山地剥', symbol: '䷖', upperTrigram: GEN, lowerTrigram: KUN,
        judgment: '剥：不利有攸往。',
        lines: [
            { position: 1, text: '初六：剥床以足，蔑贞凶。' },
            { position: 2, text: '六二：剥床以辨，蔑贞凶。' },
            { position: 3, text: '六三：剥之，无咎。' },
            { position: 4, text: '六四：剥床以肤，凶。' },
            { position: 5, text: '六五：贯鱼，以宫人宠，无不利。' },
            { position: 6, text: '上九：硕果不食，君子得舆，小人剥庐。' },
        ],
    },
    {
        id: 24, name: '地雷复', symbol: '䷗', upperTrigram: KUN, lowerTrigram: ZHEN,
        judgment: '复：亨。出入无疾，朋来无咎。反复其道，七日来复，利有攸往。',
        lines: [
            { position: 1, text: '初九：不远复，无祗悔，元吉。' },
            { position: 2, text: '六二：休复，吉。' },
            { position: 3, text: '六三：频复，厉无咎。' },
            { position: 4, text: '六四：中行独复。' },
            { position: 5, text: '六五：敦复，无悔。' },
            { position: 6, text: '上六：迷复，凶，有灾眚。用行师，终有大败，以其国君，凶；至于十年，不克征。' },
        ],
    },
    {
        id: 25, name: '天雷无妄', symbol: '䷘', upperTrigram: QIAN, lowerTrigram: ZHEN,
        judgment: '无妄：元，亨，利，贞。其匪正有眚，不利有攸往。',
        lines: [
            { position: 1, text: '初九：无妄，往吉。' },
            { position: 2, text: '六二：不耕获，不菑畲，则利有攸往。' },
            { position: 3, text: '六三：无妄之灾，或系之牛，行人之得，邑人之灾。' },
            { position: 4, text: '九四：可贞，无咎。' },
            { position: 5, text: '九五：无妄之疾，勿药有喜。' },
            { position: 6, text: '上九：无妄，行有眚，无攸利。' },
        ],
    },
    {
        id: 26, name: '山天大畜', symbol: '䷙', upperTrigram: GEN, lowerTrigram: QIAN,
        judgment: '大畜：利贞，不家食吉，利涉大川。',
        lines: [
            { position: 1, text: '初九：有厉，利已。' },
            { position: 2, text: '九二：舆说輹。' },
            { position: 3, text: '九三：良马逐，利艰贞。曰闲舆卫，利有攸往。' },
            { position: 4, text: '六四：童牛之牿，元吉。' },
            { position: 5, text: '六五：豮豕之牙，吉。' },
            { position: 6, text: '上九：何天之衢，亨。' },
        ],
    },
    {
        id: 27, name: '山雷颐', symbol: '䷚', upperTrigram: GEN, lowerTrigram: ZHEN,
        judgment: '颐：贞吉。观颐，自求口实。',
        lines: [
            { position: 1, text: '初九：舍尔灵龟，观我朵颐，凶。' },
            { position: 2, text: '六二：颠颐，拂经，于丘颐，征凶。' },
            { position: 3, text: '六三：拂颐，贞凶，十年勿用，无攸利。' },
            { position: 4, text: '六四：颠颐吉，虎视眈眈，其欲逐逐，无咎。' },
            { position: 5, text: '六五：拂经，居贞吉，不可涉大川。' },
            { position: 6, text: '上九：由颐，厉吉，利涉大川。' },
        ],
    },
    {
        id: 28, name: '泽风大过', symbol: '䷛', upperTrigram: DUI, lowerTrigram: XUN,
        judgment: '大过：栋桡，利有攸往，亨。',
        lines: [
            { position: 1, text: '初六：藉用白茅，无咎。' },
            { position: 2, text: '九二：枯杨生稊，老夫得其女妻，无不利。' },
            { position: 3, text: '九三：栋桡，凶。' },
            { position: 4, text: '九四：栋隆，吉；有它吝。' },
            { position: 5, text: '九五：枯杨生华，老妇得士夫，无咎无誉。' },
            { position: 6, text: '上六：过涉灭顶，凶，无咎。' },
        ],
    },
    {
        id: 29, name: '坎为水', symbol: '䷜', upperTrigram: KAN, lowerTrigram: KAN,
        judgment: '习坎：有孚，维心亨，行有尚。',
        lines: [
            { position: 1, text: '初六：习坎，入于坎窞，凶。' },
            { position: 2, text: '九二：坎有险，求小得。' },
            { position: 3, text: '六三：来之坎坎，险且枕，入于坎窞，勿用。' },
            { position: 4, text: '六四：樽酒簋贰，用缶，纳约自牖，终无咎。' },
            { position: 5, text: '九五：坎不盈，祗既平，无咎。' },
            { position: 6, text: '上六：系用徽纆，寘于丛棘，三岁不得，凶。' },
        ],
    },
    {
        id: 30, name: '离为火', symbol: '䷝', upperTrigram: LI, lowerTrigram: LI,
        judgment: '离：利贞，亨。畜牝牛，吉。',
        lines: [
            { position: 1, text: '初九：履错然，敬之无咎。' },
            { position: 2, text: '六二：黄离，元吉。' },
            { position: 3, text: '九三：日昃之离，不鼓缶而歌，则大耋之嗟，凶。' },
            { position: 4, text: '九四：突如其来如，焚如，死如，弃如。' },
            { position: 5, text: '六五：出涕沱若，戚嗟若，吉。' },
            { position: 6, text: '上九：王用出征，有嘉折首，获其匪丑，无咎。' },
        ],
    },
    {
        id: 31, name: '泽山咸', symbol: '䷞', upperTrigram: DUI, lowerTrigram: GEN,
        judgment: '咸：亨，利贞，取女吉。',
        lines: [
            { position: 1, text: '初六：咸其拇。' },
            { position: 2, text: '六二：咸其腓，凶，居吉。' },
            { position: 3, text: '九三：咸其股，执其随，往吝。' },
            { position: 4, text: '九四：贞吉悔亡，憧憧往来，朋从尔思。' },
            { position: 5, text: '九五：咸其脢，无悔。' },
            { position: 6, text: '上六：咸其辅颊舌。' },
        ],
    },
    {
        id: 32, name: '雷风恒', symbol: '䷟', upperTrigram: ZHEN, lowerTrigram: XUN,
        judgment: '恒：亨，无咎，利贞，利有攸往。',
        lines: [
            { position: 1, text: '初六：浚恒，贞凶，无攸利。' },
            { position: 2, text: '九二：悔亡。' },
            { position: 3, text: '九三：不恒其德，或承之羞，贞吝。' },
            { position: 4, text: '九四：田无禽。' },
            { position: 5, text: '六五：恒其德，贞，妇人吉，夫子凶。' },
            { position: 6, text: '上六：振恒，凶。' },
        ],
    },
    {
        id: 33, name: '天山遁', symbol: '䷠', upperTrigram: QIAN, lowerTrigram: GEN,
        judgment: '遁：亨，小利贞。',
        lines: [
            { position: 1, text: '初六：遁尾，厉，勿用有攸往。' },
            { position: 2, text: '六二：执之用黄牛之革，莫之胜说。' },
            { position: 3, text: '九三：系遁，有疾厉，畜臣妾吉。' },
            { position: 4, text: '九四：好遁，君子吉，小人否。' },
            { position: 5, text: '九五：嘉遁，贞吉。' },
            { position: 6, text: '上九：肥遁，无不利。' },
        ],
    },
    {
        id: 34, name: '雷天大壮', symbol: '䷡', upperTrigram: ZHEN, lowerTrigram: QIAN,
        judgment: '大壮：利贞。',
        lines: [
            { position: 1, text: '初九：壮于趾，征凶，有孚。' },
            { position: 2, text: '九二：贞吉。' },
            { position: 3, text: '九三：小人用壮，君子用罔，贞厉。羝羊触藩，羸其角。' },
            { position: 4, text: '九四：贞吉悔亡，藩决不羸，壮于大舆之輹。' },
            { position: 5, text: '六五：丧羊于易，无悔。' },
            { position: 6, text: '上六：羝羊触藩，不能退，不能遂，无攸利，艰则吉。' },
        ],
    },
    {
        id: 35, name: '火地晋', symbol: '䷢', upperTrigram: LI, lowerTrigram: KUN,
        judgment: '晋：康侯用锡马蕃庶，昼日三接。',
        lines: [
            { position: 1, text: '初六：晋如，摧如，贞吉。罔孚，裕无咎。' },
            { position: 2, text: '六二：晋如，愁如，贞吉。受兹介福，于其王母。' },
            { position: 3, text: '六三：众允，悔亡。' },
            { position: 4, text: '九四：晋如硕鼠，贞厉。' },
            { position: 5, text: '六五：悔亡，失得勿恤，往吉无不利。' },
            { position: 6, text: '上九：晋其角，维用伐邑，厉吉无咎，贞吝。' },
        ],
    },
    {
        id: 36, name: '地火明夷', symbol: '䷣', upperTrigram: KUN, lowerTrigram: LI,
        judgment: '明夷：利艰贞。',
        lines: [
            { position: 1, text: '初九：明夷于飞，垂其翼。君子于行，三日不食，有攸往，主人有言。' },
            { position: 2, text: '六二：明夷，夷于左股，用拯马壮，吉。' },
            { position: 3, text: '九三：明夷于南狩，得其大首，不可疾贞。' },
            { position: 4, text: '六四：入于左腹，获明夷之心，于出门庭。' },
            { position: 5, text: '六五：箕子之明夷，利贞。' },
            { position: 6, text: '上六：不明晦，初登于天，后入于地。' },
        ],
    },
    {
        id: 37, name: '风火家人', symbol: '䷤', upperTrigram: XUN, lowerTrigram: LI,
        judgment: '家人：利女贞。',
        lines: [
            { position: 1, text: '初九：闲有家，悔亡。' },
            { position: 2, text: '六二：无攸遂，在中馈，贞吉。' },
            { position: 3, text: '九三：家人嗃嗃，悔厉吉；妇子嘻嘻，终吝。' },
            { position: 4, text: '六四：富家，大吉。' },
            { position: 5, text: '九五：王假有家，勿恤吉。' },
            { position: 6, text: '上九：有孚威如，终吉。' },
        ],
    },
    {
        id: 38, name: '火泽睽', symbol: '䷥', upperTrigram: LI, lowerTrigram: DUI,
        judgment: '睽：小事吉。',
        lines: [
            { position: 1, text: '初九：悔亡，丧马勿逐，自复；见恶人无咎。' },
            { position: 2, text: '九二：遇主于巷，无咎。' },
            { position: 3, text: '六三：见舆曳，其牛掣，其人天且劓，无初有终。' },
            { position: 4, text: '九四：睽孤，遇元夫，交孚，厉无咎。' },
            { position: 5, text: '六五：悔亡，厥宗噬肤，往何咎。' },
            { position: 6, text: '上九：睽孤，见豕负涂，载鬼一车，先张之弧，后说之弧，匪寇婚媾，往遇雨则吉。' },
        ],
    },
    {
        id: 39, name: '水山蹇', symbol: '䷦', upperTrigram: KAN, lowerTrigram: GEN,
        judgment: '蹇：利西南，不利东北；利见大人，贞吉。',
        lines: [
            { position: 1, text: '初六：往蹇，来誉。' },
            { position: 2, text: '六二：王臣蹇蹇，匪躬之故。' },
            { position: 3, text: '九三：往蹇来反。' },
            { position: 4, text: '六四：往蹇来连。' },
            { position: 5, text: '九五：大蹇朋来。' },
            { position: 6, text: '上六：往蹇来硕，吉；利见大人。' },
        ],
    },
    {
        id: 40, name: '雷水解', symbol: '䷧', upperTrigram: ZHEN, lowerTrigram: KAN,
        judgment: '解：利西南，无所往，其来复吉。有攸往，夙吉。',
        lines: [
            { position: 1, text: '初六：无咎。' },
            { position: 2, text: '九二：田获三狐，得黄矢，贞吉。' },
            { position: 3, text: '六三：负且乘，致寇至，贞吝。' },
            { position: 4, text: '九四：解而拇，朋至斯孚。' },
            { position: 5, text: '六五：君子维有解，吉；有孚于小人。' },
            { position: 6, text: '上六：公用射隼于高墉之上，获之，无不利。' },
        ],
    },
    {
        id: 41, name: '山泽损', symbol: '䷨', upperTrigram: GEN, lowerTrigram: DUI,
        judgment: '损：有孚，元吉，无咎，可贞，利有攸往。曷之用，二簋可用享。',
        lines: [
            { position: 1, text: '初九：已事遄往，无咎，酌损之。' },
            { position: 2, text: '九二：利贞，征凶，弗损益之。' },
            { position: 3, text: '六三：三人行，则损一人；一人行，则得其友。' },
            { position: 4, text: '六四：损其疾，使遄有喜，无咎。' },
            { position: 5, text: '六五：或益之，十朋之龟弗克违，元吉。' },
            { position: 6, text: '上九：弗损益之，无咎，贞吉，利有攸往，得臣无家。' },
        ],
    },
    {
        id: 42, name: '风雷益', symbol: '䷩', upperTrigram: XUN, lowerTrigram: ZHEN,
        judgment: '益：利有攸往，利涉大川。',
        lines: [
            { position: 1, text: '初九：利用为大作，元吉，无咎。' },
            { position: 2, text: '六二：或益之，十朋之龟弗克违，永贞吉。王用享于帝，吉。' },
            { position: 3, text: '六三：益之用凶事，无咎。有孚中行，告公用圭。' },
            { position: 4, text: '六四：中行，告公从。利用为依迁国。' },
            { position: 5, text: '九五：有孚惠心，勿问元吉。有孚惠我德。' },
            { position: 6, text: '上九：莫益之，或击之，立心勿恒，凶。' },
        ],
    },
    {
        id: 43, name: '泽天夬', symbol: '䷪', upperTrigram: DUI, lowerTrigram: QIAN,
        judgment: '夬：扬于王庭，孚号，有厉，告自邑，不利即戎，利有攸往。',
        lines: [
            { position: 1, text: '初九：壮于前趾，往不胜为吝。' },
            { position: 2, text: '九二：惕号，莫夜有戎，勿恤。' },
            { position: 3, text: '九三：壮于頄，有凶。君子夬夬，独行遇雨，若濡有愠，无咎。' },
            { position: 4, text: '九四：臀无肤，其行次且。牵羊悔亡，闻言不信。' },
            { position: 5, text: '九五：苋陆夬夬，中行无咎。' },
            { position: 6, text: '上六：无号，终有凶。' },
        ],
    },
    {
        id: 44, name: '天风姤', symbol: '䷫', upperTrigram: QIAN, lowerTrigram: XUN,
        judgment: '姤：女壮，勿用取女。',
        lines: [
            { position: 1, text: '初六：系于金柅，贞吉，有攸往，见凶，羸豕孚蹢躅。' },
            { position: 2, text: '九二：包有鱼，无咎，不利宾。' },
            { position: 3, text: '九三：臀无肤，其行次且，厉，无大咎。' },
            { position: 4, text: '九四：包无鱼，起凶。' },
            { position: 5, text: '九五：以杞包瓜，含章，有陨自天。' },
            { position: 6, text: '上九：姤其角，吝，无咎。' },
        ],
    },
    {
        id: 45, name: '泽地萃', symbol: '䷬', upperTrigram: DUI, lowerTrigram: KUN,
        judgment: '萃：亨。王假有庙，利见大人，亨，利贞。用大牲吉，利有攸往。',
        lines: [
            { position: 1, text: '初六：有孚不终，乃乱乃萃。若号，一握为笑，勿恤，往无咎。' },
            { position: 2, text: '六二：引吉，无咎，孚乃利用禴。' },
            { position: 3, text: '六三：萃如，嗟如，无攸利，往无咎，小吝。' },
            { position: 4, text: '九四：大吉，无咎。' },
            { position: 5, text: '九五：萃有位，无咎。匪孚，元永贞，悔亡。' },
            { position: 6, text: '上六：赍咨涕洟，无咎。' },
        ],
    },
    {
        id: 46, name: '地风升', symbol: '䷭', upperTrigram: KUN, lowerTrigram: XUN,
        judgment: '升：元亨，用见大人，勿恤，南征吉。',
        lines: [
            { position: 1, text: '初六：允升，大吉。' },
            { position: 2, text: '九二：孚乃利用禴，无咎。' },
            { position: 3, text: '九三：升虚邑。' },
            { position: 4, text: '六四：王用亨于岐山，吉无咎。' },
            { position: 5, text: '六五：贞吉，升阶。' },
            { position: 6, text: '上六：冥升，利于不息之贞。' },
        ],
    },
    {
        id: 47, name: '泽水困', symbol: '䷮', upperTrigram: DUI, lowerTrigram: KAN,
        judgment: '困：亨，贞，大人吉，无咎，有言不信。',
        lines: [
            { position: 1, text: '初六：臀困于株木，入于幽谷，三岁不见。' },
            { position: 2, text: '九二：困于酒食，朱绂方来，利用亨祀，征凶，无咎。' },
            { position: 3, text: '六三：困于石，据于蒺藜，入于其宫，不见其妻，凶。' },
            { position: 4, text: '九四：来徐徐，困于金车，吝，有终。' },
            { position: 5, text: '九五：劓刖，困于赤绂，乃徐有说，利用祭祀。' },
            { position: 6, text: '上六：困于葛藟，于臲卼，曰动悔。有悔，征吉。' },
        ],
    },
    {
        id: 48, name: '水风井', symbol: '䷯', upperTrigram: KAN, lowerTrigram: XUN,
        judgment: '井：改邑不改井，无丧无得，往来井井。汔至，亦未繘井，羸其瓶，凶。',
        lines: [
            { position: 1, text: '初六：井泥不食，旧井无禽。' },
            { position: 2, text: '九二：井谷射鲋，瓮敝漏。' },
            { position: 3, text: '九三：井渫不食，为我民恻，可用汲，王明，并受其福。' },
            { position: 4, text: '六四：井甃，无咎。' },
            { position: 5, text: '九五：井冽，寒泉食。' },
            { position: 6, text: '上六：井收勿幕，有孚元吉。' },
        ],
    },
    {
        id: 49, name: '泽火革', symbol: '䷰', upperTrigram: DUI, lowerTrigram: LI,
        judgment: '革：己日乃孚，元亨利贞，悔亡。',
        lines: [
            { position: 1, text: '初九：巩用黄牛之革。' },
            { position: 2, text: '六二：己日乃革之，征吉，无咎。' },
            { position: 3, text: '九三：征凶，贞厉，革言三就，有孚。' },
            { position: 4, text: '九四：悔亡，有孚改命，吉。' },
            { position: 5, text: '九五：大人虎变，未占有孚。' },
            { position: 6, text: '上六：君子豹变，小人革面，征凶，居贞吉。' },
        ],
    },
    {
        id: 50, name: '火风鼎', symbol: '䷱', upperTrigram: LI, lowerTrigram: XUN,
        judgment: '鼎：元吉，亨。',
        lines: [
            { position: 1, text: '初六：鼎颠趾，利出否，得妾以其子，无咎。' },
            { position: 2, text: '九二：鼎有实，我仇有疾，不我能即，吉。' },
            { position: 3, text: '九三：鼎耳革，其行塞，雉膏不食，方雨亏悔，终吉。' },
            { position: 4, text: '九四：鼎折足，覆公餗，其形渥，凶。' },
            { position: 5, text: '六五：鼎黄耳金铉，利贞。' },
            { position: 6, text: '上九：鼎玉铉，大吉，无不利。' },
        ],
    },
    {
        id: 51, name: '震为雷', symbol: '䷲', upperTrigram: ZHEN, lowerTrigram: ZHEN,
        judgment: '震：亨。震来虩虩，笑言哑哑。震惊百里，不丧匕鬯。',
        lines: [
            { position: 1, text: '初九：震来虩虩，后笑言哑哑，吉。' },
            { position: 2, text: '六二：震来厉，亿丧贝，跻于九陵，勿逐，七日得。' },
            { position: 3, text: '六三：震苏苏，震行无眚。' },
            { position: 4, text: '九四：震遂泥。' },
            { position: 5, text: '六五：震往来厉，亿无丧，有事。' },
            { position: 6, text: '上六：震索索，视矍矍，征凶。震不于其躬，于其邻，无咎。婚媾有言。' },
        ],
    },
    {
        id: 52, name: '艮为山', symbol: '䷳', upperTrigram: GEN, lowerTrigram: GEN,
        judgment: '艮：艮其背，不获其身，行其庭，不见其人，无咎。',
        lines: [
            { position: 1, text: '初六：艮其趾，无咎，利永贞。' },
            { position: 2, text: '六二：艮其腓，不拯其随，其心不快。' },
            { position: 3, text: '九三：艮其限，列其夤，厉薰心。' },
            { position: 4, text: '六四：艮其身，无咎。' },
            { position: 5, text: '六五：艮其辅，言有序，悔亡。' },
            { position: 6, text: '上九：敦艮，吉。' },
        ],
    },
    {
        id: 53, name: '风山渐', symbol: '䷴', upperTrigram: XUN, lowerTrigram: GEN,
        judgment: '渐：女归吉，利贞。',
        lines: [
            { position: 1, text: '初六：鸿渐于干，小子厉，有言，无咎。' },
            { position: 2, text: '六二：鸿渐于磐，饮食衎衎，吉。' },
            { position: 3, text: '九三：鸿渐于陆，夫征不复，妇孕不育，凶；利御寇。' },
            { position: 4, text: '六四：鸿渐于木，或得其桷，无咎。' },
            { position: 5, text: '九五：鸿渐于陵，妇三岁不孕，终莫之胜，吉。' },
            { position: 6, text: '上九：鸿渐于陆，其羽可用为仪，吉。' },
        ],
    },
    {
        id: 54, name: '雷泽归妹', symbol: '䷵', upperTrigram: ZHEN, lowerTrigram: DUI,
        judgment: '归妹：征凶，无攸利。',
        lines: [
            { position: 1, text: '初九：归妹以娣，跛能履，征吉。' },
            { position: 2, text: '九二：眇能视，利幽人之贞。' },
            { position: 3, text: '六三：归妹以须，反归以娣。' },
            { position: 4, text: '九四：归妹愆期，迟归有时。' },
            { position: 5, text: '六五：帝乙归妹，其君之袂，不如其娣之袂良，月几望，吉。' },
            { position: 6, text: '上六：女承筐无实，士刲羊无血，无攸利。' },
        ],
    },
    {
        id: 55, name: '雷火丰', symbol: '䷶', upperTrigram: ZHEN, lowerTrigram: LI,
        judgment: '丰：亨，王假之，勿忧，宜日中。',
        lines: [
            { position: 1, text: '初九：遇其配主，虽旬无咎，往有尚。' },
            { position: 2, text: '六二：丰其蔀，日中见斗，往得疑疾，有孚发若，吉。' },
            { position: 3, text: '九三：丰其沛，日中见沬，折其右肱，无咎。' },
            { position: 4, text: '九四：丰其蔀，日中见斗，遇其夷主，吉。' },
            { position: 5, text: '六五：来章，有庆誉，吉。' },
            { position: 6, text: '上六：丰其屋，蔀其家，窥其户，阒其无人，三岁不见，凶。' },
        ],
    },
    {
        id: 56, name: '火山旅', symbol: '䷷', upperTrigram: LI, lowerTrigram: GEN,
        judgment: '旅：小亨，旅贞吉。',
        lines: [
            { position: 1, text: '初六：旅琐琐，斯其所取灾。' },
            { position: 2, text: '六二：旅即次，怀其资，得童仆贞。' },
            { position: 3, text: '九三：旅焚其次，丧其童仆，贞厉。' },
            { position: 4, text: '九四：旅于处，得其资斧，我心不快。' },
            { position: 5, text: '六五：射雉一矢亡，终以誉命。' },
            { position: 6, text: '上九：鸟焚其巢，旅人先笑后号啕。丧牛于易，凶。' },
        ],
    },
    {
        id: 57, name: '巽为风', symbol: '䷸', upperTrigram: XUN, lowerTrigram: XUN,
        judgment: '巽：小亨，利有攸往，利见大人。',
        lines: [
            { position: 1, text: '初六：进退，利武人之贞。' },
            { position: 2, text: '九二：巽在床下，用史巫纷若，吉无咎。' },
            { position: 3, text: '九三：频巽，吝。' },
            { position: 4, text: '六四：悔亡，田获三品。' },
            { position: 5, text: '九五：贞吉悔亡，无不利。无初有终，先庚三日，后庚三日，吉。' },
            { position: 6, text: '上九：巽在床下，丧其资斧，贞凶。' },
        ],
    },
    {
        id: 58, name: '兑为泽', symbol: '䷹', upperTrigram: DUI, lowerTrigram: DUI,
        judgment: '兑：亨，利贞。',
        lines: [
            { position: 1, text: '初九：和兑，吉。' },
            { position: 2, text: '九二：孚兑，吉，悔亡。' },
            { position: 3, text: '六三：来兑，凶。' },
            { position: 4, text: '九四：商兑，未宁，介疾有喜。' },
            { position: 5, text: '九五：孚于剥，有厉。' },
            { position: 6, text: '上六：引兑。' },
        ],
    },
    {
        id: 59, name: '风水涣', symbol: '䷺', upperTrigram: XUN, lowerTrigram: KAN,
        judgment: '涣：亨。王假有庙，利涉大川，利贞。',
        lines: [
            { position: 1, text: '初六：用拯马壮，吉。' },
            { position: 2, text: '九二：涣奔其机，悔亡。' },
            { position: 3, text: '六三：涣其躬，无悔。' },
            { position: 4, text: '六四：涣其群，元吉。涣有丘，匪夷所思。' },
            { position: 5, text: '九五：涣汗其大号，涣王居，无咎。' },
            { position: 6, text: '上九：涣其血，去逖出，无咎。' },
        ],
    },
    {
        id: 60, name: '水泽节', symbol: '䷻', upperTrigram: KAN, lowerTrigram: DUI,
        judgment: '节：亨。苦节不可贞。',
        lines: [
            { position: 1, text: '初九：不出户庭，无咎。' },
            { position: 2, text: '九二：不出门庭，凶。' },
            { position: 3, text: '六三：不节若，则嗟若，无咎。' },
            { position: 4, text: '六四：安节，亨。' },
            { position: 5, text: '九五：甘节，吉；往有尚。' },
            { position: 6, text: '上六：苦节，贞凶，悔亡。' },
        ],
    },
    {
        id: 61, name: '风泽中孚', symbol: '䷼', upperTrigram: XUN, lowerTrigram: DUI,
        judgment: '中孚：豚鱼吉，利涉大川，利贞。',
        lines: [
            { position: 1, text: '初九：虞吉，有他不燕。' },
            { position: 2, text: '九二：鸣鹤在阴，其子和之，我有好爵，吾与尔靡之。' },
            { position: 3, text: '六三：得敌，或鼓或罢，或泣或歌。' },
            { position: 4, text: '六四：月几望，马匹亡，无咎。' },
            { position: 5, text: '九五：有孚挛如，无咎。' },
            { position: 6, text: '上九：翰音登于天，贞凶。' },
        ],
    },
    {
        id: 62, name: '雷山小过', symbol: '䷽', upperTrigram: ZHEN, lowerTrigram: GEN,
        judgment: '小过：亨，利贞，可小事，不可大事。飞鸟遗之音，不宜上宜下，大吉。',
        lines: [
            { position: 1, text: '初六：飞鸟以凶。' },
            { position: 2, text: '六二：过其祖，遇其妣；不及其君，遇其臣；无咎。' },
            { position: 3, text: '九三：弗过防之，从或戕之，凶。' },
            { position: 4, text: '九四：无咎，弗过遇之。往厉必戒，勿用永贞。' },
            { position: 5, text: '六五：密云不雨，自我西郊，公弋取彼在穴。' },
            { position: 6, text: '上六：弗遇过之，飞鸟离之，凶，是谓灾眚。' },
        ],
    },
    {
        id: 63, name: '水火既济', symbol: '䷾', upperTrigram: KAN, lowerTrigram: LI,
        judgment: '既济：亨，小利贞，初吉终乱。',
        lines: [
            { position: 1, text: '初九：曳其轮，濡其尾，无咎。' },
            { position: 2, text: '六二：妇丧其茀，勿逐，七日得。' },
            { position: 3, text: '九三：高宗伐鬼方，三年克之，小人勿用。' },
            { position: 4, text: '六四：繻有衣袽，终日戒。' },
            { position: 5, text: '九五：东邻杀牛，不如西邻之禴祭，实受其福。' },
            { position: 6, text: '上六：濡其首，厉。' },
        ],
    },
    {
        id: 64, name: '火水未济', symbol: '䷿', upperTrigram: LI, lowerTrigram: KAN,
        judgment: '未济：亨，小狐汔济，濡其尾，无攸利。',
        lines: [
            { position: 1, text: '初六：濡其尾，吝。' },
            { position: 2, text: '九二：曳其轮，贞吉。' },
            { position: 3, text: '六三：未济，征凶，利涉大川。' },
            { position: 4, text: '九四：贞吉悔亡，震用伐鬼方，三年有赏于大国。' },
            { position: 5, text: '六五：贞吉，无悔，君子之光，有孚，吉。' },
            { position: 6, text: '上九：有孚于饮酒，无咎。濡其首，有孚失是。' },
        ],
    },
];
function getHexagramById(id) {
    return exports.HEXAGRAMS.find(h => h.id === id) || exports.HEXAGRAMS[0];
}
// 六十四卦白话释义（吉凶倾向与行事提示），用于卦象解析断语，
// 与经典卦辞、爻辞互为补充而不重复照录原文。
const HEXAGRAM_INTERPRETATIONS = {
    1: '刚健中正，自强不息。大吉大利，宜积极进取；然物极必反，盛极当防骄亢。',
    2: '厚德载物，柔顺包容。利于守静顺承、辅佐他人，先迷后得，宜静不宜先动。',
    3: '万事起头难，如草木初生。艰险交加，宜守正待时、积蓄实力，不可贸然前行。',
    4: '蒙昧未开，如童稚待教。宜虚心求教、启蒙开智，诚心问则通，反复轻慢则不应。',
    5: '云上于天，待时而动。宜耐心守候、以待其时，不可急躁冒进，守正有孚则吉。',
    6: '上下相争，是非口舌。争讼多凶，宜和解退让、慎始防患，得中则吉，终争则凶。',
    7: '兴兵动众，以正为本。宜师出有名、任用贤能，纪律严明则吉，妄动轻进则险。',
    8: '亲密团结，相互依附。宜择善而从、以诚相亲，先者得吉，迟疑观望则失机。',
    9: '小有积蓄，力量未足。宜以柔畜刚、暂缓待时，密云不雨，尚需继续积累。',
    10: '如履虎尾，谨慎前行。行事须循礼守分、小心谨慎，柔顺得中则化险为夷。',
    11: '天地交泰，通达安康。上下和顺、诸事亨通，大吉之象；然须守成，防泰极转否。',
    12: '天地不交，闭塞不通。诸事阻滞、小人道长，宜守正自保、静待转机，否极终泰来。',
    13: '与人同心，和同于众。宜开诚布公、协力合作，得众人相助则大事可成。',
    14: '富有丰盛，柔得尊位。事业昌隆、所获丰厚，宜谦和守正、居安思危方能长保。',
    15: '谦谦君子，卑以自牧。谦逊有终、诸事皆吉，为难得的纯吉之卦，宜始终守谦德。',
    16: '顺时而动，和乐安逸。宜顺势而为、未雨绸缪；然逸乐当有节，不可耽溺忘忧。',
    17: '随顺时势，择善而从。宜顺应变化、随时而动，守正相随则亨，随非其人则失。',
    18: '积弊待治，拨乱反正。事有沉疴、宜革故除弊，整治须及时，先甲后甲方得吉。',
    19: '阳气渐长，临事而进。宜把握时机、积极临事；然盛极将衰，至八月有凶，宜早筹谋。',
    20: '观仰瞻望，以德化人。宜静观审察、修身立德，观照全局而后动，不宜妄动躁进。',
    21: '咬合去梗，明断刑罚。事有阻梗、宜果决排除，明辨是非、恩威并施则通达。',
    22: '文饰装点，修饰有度。宜注重仪表文采；然文过则饰伪，质朴为本，小事可为。',
    23: '阴盛阳衰，剥落将尽。诸事衰败、不利有所往，宜顺时止步、静守待变，切忌强进。',
    24: '一阳来复，否极泰来。生机重现、渐入佳境，宜顺应回归、把握新机，出入无疾。',
    25: '无妄而行，真实无欺。宜守正循理、不存妄念，顺其自然则吉，心怀妄念则招灾。',
    26: '大有积蓄，刚健笃实。蓄势深厚、养精待发，宜储才养德、任用贤能，利涉大川。',
    27: '颐养之道，慎言节食。宜谨言慎行、修身养正，观其所养、自求口实则得吉。',
    28: '大为过甚，栋梁将挠。事已过极、负荷过重，宜权变应急、稳健处之，不可逞强。',
    29: '重险叠陷，水流不盈。险难重重、身处困境，宜守信持正、险中求进，习坎方能出险。',
    30: '附丽光明，柔顺守正。宜依附正道、明察秋毫，守持中正则亨，畜养柔顺则获吉。',
    31: '交感相应，两情相通。宜以诚相感、虚心待人，男女相感则吉，尤利婚姻交往。',
    32: '恒久不变，持之以恒。宜守常持恒、始终如一，坚守正道则亨；然不可固执不知变通。',
    33: '见几而退，明哲保身。时势不利、宜急流勇退，退避守正则吉，恋栈不去则招危。',
    34: '阳刚壮盛，气势正旺。声势浩大、宜守正而行；然壮而不已则有悔，戒骄戒躁为要。',
    35: '日出地上，光明上进。前程光明、晋升有望，宜积极进取、以德受福，为大吉之象。',
    36: '明入地中，光明受损。贤者遭难、时局晦暗，宜韬光养晦、守正待时，内明外柔以避害。',
    37: '家道齐整，各安其分。宜正家立业、各尽其责，家人和睦、利于守持则得吉庆。',
    38: '乖离背违，同中有异。人事乖违、意见相左，宜求同存异、以柔济刚，小事尚可为。',
    39: '前险后阻，行路艰难。险阻当前、举步维艰，宜见险而止、反身修德，利见大人则解。',
    40: '险难消解，缓解舒展。困境将解、宜把握时机，赦过宥罪、及时行动则获吉。',
    41: '损下益上，减损有道。虽有所损、然损中有益，宜诚心节俭、损己利人，终得吉庆。',
    42: '损上益下，增益获利。利益增长、诸事有利，宜把握良机、积极进取，利涉大川。',
    43: '决断刚决，除恶务尽。宜果决除弊、明扬于庭；然须以正制邪，不可专恃刚强。',
    44: '不期而遇，阴长遇阳。虽有意外相逢，然一阴始生、当防小人，女壮勿娶，不可轻信。',
    45: '荟萃聚集，人心归聚。宜聚合众力、择位而处，诚敬则通，利见大人，为大吉之象。',
    46: '柔顺上升，积小成大。宜循序渐进、积累而升，见大人则吉，前程可期、勿疑而进。',
    47: '困顿受制，处境艰难。身陷困境、言而无信人，宜守正安命、以静制困，君子固穷则亨。',
    48: '井养不穷，德泽及人。宜修养自身、惠泽于人，井道贵在恒常，守德不移则得吉。',
    49: '变革更新，除旧布新。时势当变、宜顺天应人，革之有时、孚信而行则亨，其悔乃亡。',
    50: '鼎新取象，稳重承载。宜革故鼎新、稳固根基，任贤纳福、调和鼎鼐则元亨大吉。',
    51: '震动惊惧，动中求安。震惊百里、宜临危不乱，恐惧修省则吉，处变不惊者得福。',
    52: '抑止安静，适可而止。宜知止而止、安分守己，动静得时、不妄躁进则无咎。',
    53: '循序渐进，稳步而行。宜按部就班、稳中求进，如女归得正，渐进则吉、躁进则乱。',
    54: '归嫁失正，行事欠宜。事有不当、名不正则言不顺，宜守正自持、慎处进退，妄动则凶。',
    55: '丰盛盛大，光明壮阔。事业丰隆、如日中天，宜把握鼎盛、居安思危；日中则昃，盛极防衰。',
    56: '羁旅在外，漂泊不定。身处旅途、根基未稳，宜柔顺谨慎、安分守己，小事可为、不宜大举。',
    57: '谦逊随顺，无孔不入。宜柔顺谦逊、申命行事，如随风潜入、渐进则通，利见大人。',
    58: '和悦欢欣，以悦待人。宜和颜悦色、以诚相悦；然悦须守正，谄媚取悦则失，守正则亨。',
    59: '涣散离析，宜聚人心。人心涣散、当涣中求聚，以诚聚合、共赴其事，利涉大川则吉。',
    60: '节制有度，适可而止。宜节俭自律、守之以度；然节不可过苦，苦节难久，中节则通。',
    61: '诚信中实，感化于物。宜以诚待人、心怀笃信，诚信及于豚鱼则吉，利涉大川、守正则应。',
    62: '小有过越，宜谨慎行。可行小事、不宜大举，宜谦恭俭约、守分安常，宜下不宜上、宜守不宜进。',
    63: '事已成就，守成防变。诸事初成、然盛极易衰，宜居安思危、慎终如始，初吉终乱、防患未然。',
    64: '事未成就，慎终可成。事在将成未成之际，宜审慎持正、把握时机，如小狐涉水，慎终则可济。',
};
exports.HEXAGRAMS.forEach(h => {
    h.interpretation = HEXAGRAM_INTERPRETATIONS[h.id];
});
